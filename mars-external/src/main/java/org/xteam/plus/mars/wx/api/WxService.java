...
import org.xteam.plus.mars.wx.bean.result.*;
...
public class WxService implements IService {
    ...
    public boolean verifyWeixinNotify(Map<Object, Object> map) {
        ...
        for (Object keyValue : map.keySet()) {
            if (!keyValue.toString().equals("sign")) {
                parameterMap.put(keyValue.toString(), String.valueOf(map.get(keyValue)));
            }
        }
        String createSign = PayUtil.createSign(parameterMap, WxConfig.getInstance().getApiKey());
    ...
    @Override
    public WxUserList batchGetUserInfo(List<WxUserList.WxUser.WxUserGet> usersGet) throws WxErrorException {
    ...
    public InvokePay unifiedOrder(PayOrderInfo order, String notifyUrl, String openid) throws WxErrorException {
        ...
        ivp.setCodeUrl(result.getCodeUrl());
        //拼接map
